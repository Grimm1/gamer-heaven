...
function gamer_heaven_login_styles() {
    ...
    // Inline CSS to override core language switcher styles
    $custom_css .= '
        ...
        body.login div.language-switcher form {
            display: flex !important;
            justify-content: center !important;
            align-items: center !important;
            gap: 5px !important;
        ...
        @media (max-width: 600px) {
            ...
            body.login div.language-switcher input[type="submit"] {
...
add_action('login_enqueue_scripts', 'gamer_heaven_login_styles', 999);

/**
 * Enqueue script to move language switcher into #login
 */
function gamer_heaven_enqueue_language_switcher_script() {
    $script_path = get_template_directory() . '/js/language-switcher.js';
    if (file_exists($script_path)) {
        wp_enqueue_script(
            'gamer-heaven-language-switcher',
            get_template_directory_uri() . '/js/language-switcher.js',
            array('jquery'),
            GAMER_HEAVEN_VERSION,
            true
        );
    }
}
add_action('login_enqueue_scripts', 'gamer_heaven_enqueue_language_switcher_script', 1000);

/**
 * Remove invalid gamer_heaven_move_language_switcher hook
 */
function gamer_heaven_cleanup_invalid_hooks() {
    remove_action('login_footer', 'gamer_heaven_move_language_switcher');
}
add_action('init', 'gamer_heaven_cleanup_invalid_hooks', 0);
 ...
 */
function gamer_heaven_sanitize_checkbox($input) {
    return (isset($input) && true === (bool) $input);
}
                            ...
                            <div class="social-repeater SINGLE-row">
                                ...
                                <label>
                                    ...
                                    <div class="social-repeater-icon-upload">
                                        <input type="hidden" class="social-repeater-icon"
                                            value="<?php echo esc_attr($link['icon']); ?>" />
                                        <img src="<?php echo $link['icon'] ? esc_url(wp_get_attachment_image_url($link['icon'], 'thumbnail')) : ''; ?>"
                                            class="social-repeater-icon-preview"
                                            style="max-width: 50px; <?php echo $link['icon'] ? '' : 'display: none;'; ?>" />
                                        <button type="button"
                                            class="button social-repeater-upload-button"><?php _e('Upload Icon', 'gamer-heaven'); ?></button>
                                        <button type="button" class="button social-repeater-remove-icon"
                                            style="<?php echo $link['icon'] ? '' : 'display: none;'; ?>"><?php _e('Remove Icon', 'gamer-heaven'); ?></button>
                                    </div>
                                </label>
                                <button type="button"
                                    class="button social-repeater-remove"><?php _e('Remove', 'gamer-heaven'); ?></button>
                            </div>
                        </div>
                        <?php
                    }
...
/**
 * Localize Customizer scripts
 */
function gamer_heaven_localize_customizer_scripts()
{
    return array(
        'ajaxurl' => admin_url('admin-ajax.php'),
        'nonce' => wp_create_nonce('gamer_heaven_customizer'),
    );
}
...
function gamer_heaven_customizer_preview_js()
{
    ...
    wp_localize_script(
        ...
        gamer_heaven_localize_customizer_scripts()
    );
}
...
{
    check_ajax_referer('gamer_heaven_customizer', 'nonce');
...
/**
 * AJAX handler for admin-only checkbox HTML
 */
    ...
    if (isset($_POST['item_id']) && is_numeric($_POST['item_id'])) {
        ...
        <p class="field-admin-only description description-wide">
            <label for="edit-menu-item-admin-only-<?php echo esc_attr($item_id); ?>">
                <input type="checkbox" id="edit-menu-item-admin-only-<?php echo esc_attr($item_id); ?>"
                    class="gamer-heaven-admin-only-checkbox" name="menu-item-admin-only[<?php echo esc_attr($item_id); ?>]"
                    <?php checked($admin_only, '1'); ?> value="1" />
...
function gamer_heaven_output_background_images()
{
    ...
    if ($left_image_id) {
        ...
        if ($left_image_url) {
            echo '<img src="' . esc_url($left_image_url) . '" class="gamer-heaven-background-image gamer-heaven-left-background" alt="">';
        } else {
            if (defined('WP_DEBUG') && WP_DEBUG) {
    ...
    if ($right_image_id) {
        ...
        if ($right_image_url) {
            echo '<img src="' . esc_url($right_image_url) . '" class="gamer-heaven-background-image gamer-heaven-right-background" alt="">';
        } else {
            if (defined('WP_DEBUG') && WP_DEBUG) {
 ...
 */
function gamer_heaven_comment_callback($comment, $args, $depth) {
    ?>
            ...
            <footer class="comment-meta">
                <div class="comment-author vcard">
                    <?php if ($args['avatar_size']) : ?>
                        <?php echo get_avatar($comment, $args['avatar_size'], '', esc_attr__('Avatar for', 'gamer-heaven') . ' ' . get_comment_author(), array('class' => 'avatar')); ?>
                    <?php endif; ?>
                    <b class="fn"><?php comment_author_link(); ?></b>
                    <span class="says"><?php esc_html_e('says:', 'gamer-heaven'); ?></span>
                </div>
                <div class="comment-metadata">
                    <a href="<?php echo esc_url(get_comment_link($comment->comment_ID)); ?>">
                        <time datetime="<?php comment_time('c'); ?>">
                            <?php
                            printf(
                                esc_html__('On %1$s at %2$s', 'gamer-heaven'),
                                get_comment_date(),
                                get_comment_time()
                            );
                            ?>
                    ...
                    <?php edit_comment_link(__('Edit', 'gamer-heaven'), '<span class="edit-link">', '</span>'); ?>
                </div>
            </footer>
            <div class="comment-content">
                <?php if ($comment->comment_approved == '0') : ?>
                    <p class="comment-awaiting-moderation"><?php esc_html_e('Your comment is awaiting moderation.', 'gamer-heaven'); ?></p>
                <?php endif; ?>
                <?php comment_text(); ?>
            </div>
            <div class="reply">
                ...
                comment_reply_link(
                    array_merge($args, array(
                        ...
                        'reply_text' => __('Reply', 'gamer-heaven'),
                        'add_below'  => 'comment',
                    ))
                );
                ?>
